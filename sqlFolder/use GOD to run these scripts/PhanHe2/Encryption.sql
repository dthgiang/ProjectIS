...
GRANT EXECUTE ON DBMS_CRYPTO TO god;
GRANT EXECUTE ON dbms_rls TO god;
grant EXECUTE on DBMS_RANDOM to god;
...
DECLARE 
    ...
    v_username NHANVIEN.USERNAME%TYPE;
BEGIN
    v_manv := :new.manv;
    v_tennv := :new.tennv;
    v_phai := :new.phai;
    ...
    :new.luong := mahoa(UTL_RAW.CAST_TO_RAW(v_luong), v_manv);
    :new.phucap := mahoa(UTL_RAW.CAST_TO_RAW(v_phucap), v_manv);
